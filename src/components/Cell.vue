<template>
    <div>
        ...
        <div v-if="!data.revealed" :class="$style.border"></div>
...
export default {
    ...
    methods: {
        ...
        clicked() {
            ...
            if(!this.gameOver && !this.localMarked)
        ...
        mark() {
            ...
            if(!this.data.revealed && (this.localMarked || (!this.localMarked && this.flags > 0))) {
                
                this.localMarked = !this.localMarked;

                this.$emit('update:flags', parseInt(this.flags) + (this.localMarked ? -1 : 1));
                this.$emit('update:marked', this.localMarked);
...
.border {
    ...
    box-shadow: 0 0 0 2px darken($base-color, 12);
    height: 1.8rem;
    margin: -1.8rem 0 0 0;
    position: relative;
    width: 1.8rem;
...
.neighborhoodBombsCount3 { color: rgb(172, 27, 27); }
.neighborhoodBombsCount4 { color: rgb(212, 115, 23); }
.neighborhoodBombsCount5 { color: rgb(161, 182, 41); }
.neighborhoodBombsCount6 { color: rgb(43, 177, 166); }
.neighborhoodBombsCount7 { color: rgb(206, 27, 223); }
.neighborhoodBombsCount8 { color: rgb(187, 59, 133); }
